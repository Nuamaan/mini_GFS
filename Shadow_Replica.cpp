...
Shadow_Replica::Shadow_Replica(std::string arg_host_url, std::string arg_vsID, std::string arg_class_id,
...
Json::Value
Shadow_Replica::CommitAbort(std::string arg_name, std::string arg_fhandle, std::string arg_chunk_index,
                            std::string arg_commitorabort)
{
  ...
  try
  {
    std::cout << "calling NFS CommitAbort" << std::endl;
...
Json::Value
Shadow_Replica::PushChunk2Replica(std::string arg_name, std::string arg_fhandle, std::string arg_chunk_index, std::string arg_chunk)
{
  HttpClient httpclient((this->host_url).c_str());
  minigfs_Client myClient(httpclient, JSONRPC_CLIENT_V2);
  Json::Value myv;
  ...
  try
  {
    std::cout << "calling NFS PushChunk2Replica" << std::endl;
...
bool Shadow_Replica::Jdump(Json::Value *mjv_ptr)
{
  return false;
}
